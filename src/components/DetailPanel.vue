...
<template>
  <div class="h-full rounded-lg border border-border bg-surface/30 backdrop-blur-sm overflow-hidden flex flex-col transition-all duration-300 shadow-sm">
    ...
    <div class="px-5 py-4 border-b border-border bg-surface/50">
      <h3 class="font-bold text-sm text-primary tracking-tight">Configuration Details</h3>
    </div>
    ...
    <div class="flex-1 overflow-auto p-6 scrollbar-thin">
      <div v-if="!provider" class="flex h-full flex-col items-center justify-center gap-4 text-muted-foreground opacity-50">
        ...
        <p class="text-sm font-medium">Select a provider to view details</p>
      </div>
      ...
      <div v-else class="space-y-8 animate-fade-in">
        ...
        <section class="space-y-4">
          ...
          <div class="grid grid-cols-[100px_1fr] gap-y-4 gap-x-4 text-sm">
            <span class="text-muted-foreground font-mono text-xs">NAME</span>
            <span class="font-medium text-primary">{{ provider.name }}</span>

            <span class="text-muted-foreground font-mono text-xs">ENDPOINT</span>
            ...
            <span class="text-muted-foreground font-mono text-xs">MODELS</span>
            ...
            <template v-if="provider.description">
              <span class="text-muted-foreground font-mono text-xs">DESC</span>
        ...
        <section v-if="model" class="space-y-4 animate-slide-up">
          ...
          <div class="grid grid-cols-[100px_1fr] gap-y-4 gap-x-4 text-sm">
            ...
            <template v-if="model.context_limit">
              ...
              <span class="font-mono text-xs">{{ formatTokens(model.context_limit) }} tokens</span>
            </template>

            <template v-if="model.output_limit">
              <span class="text-muted-foreground font-mono text-xs">OUTPUT</span>
              <span class="font-mono text-xs">{{ formatTokens(model.output_limit) }} tokens</span>
            </template>
          </div>
        </section>
      </div>
